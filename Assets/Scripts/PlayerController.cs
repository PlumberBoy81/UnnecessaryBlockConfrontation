...
public class PlayerController : MonoBehaviour
{
    ...
    {
        // Simple raycast down to check for floor
        RaycastHit2D hit = Physics2D.BoxCast(boxCol.bounds.center, boxCol.bounds.size * 0.9f, 0f, Vector2.down, 0.1f);
    ...
    public void TakeDamage(float damageAmount, int attackDirection, bool isVertical = false)
    {
        ...
        if (isVertical)
        {
            ...
            rb.linearVelocity = new Vector2(attackDirection * knockbackForce * 0.15f, knockbackForce * 1.1f);
        }
        else
        {
            // Standard attacks send up and away
            rb.linearVelocity = new Vector2(attackDirection * knockbackForce, knockbackForce * 0.5f);
        }
